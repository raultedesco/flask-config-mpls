...
class RegisterForm(FlaskForm):
    ...
    username = StringField('Username', validators=[InputRequired(
        'debe ingresar un valor'), Length(min=4, max=15)])
    password = PasswordField('Password', validators=[InputRequired(), Length(min=8, max=80)])
...
class DevicesConfigEIGRP(FlaskForm):
    id = StringField('ID')
    eigrp_process = StringField('EIGRP Process', validators=[
                                InputRequired(message='Ingrese Proceso EIGRP')])
    eigrp_network = StringField('EIGRP Network', validators=[
                                InputRequired(message='Ingrese Proceso EIGRP')])
    eigrp_mascara = StringField('EIGRP Wilcard bits', validators=[
                                InputRequired(message='Ingrese Proceso EIGRP')])


class DevicesConfigOSPF(FlaskForm):
    id = StringField('ID')
    ospf_process = StringField('OSPF Process', validators=[
                               InputRequired(message='Ingrese Proceso OSPF')])
    ospf_network = StringField('OSPF Network', validators=[
                               InputRequired(message='Ingrese Proceso OSPF')])
...
class DeviceConfigInterfaces(FlaskForm):
    id = StringField('ID')
    interface = StringField('Interface', validators=[
                            DataRequired(message='debe ingresar una interface')])
    interface_ip = StringField('Interface IP', validators=[IPAddress(
        ipv4=True, ipv6=False, message='IP Destino: IP invalida')])
    interface_mascara = StringField('Interface Mascara', validators=[DataRequired(message='Si o Si debe ingresar un numero de AS'), Length(
        min=7, max=15, message='Mascara: debe tener un minimo de 5 y un maximo de 15 caracteres')])


class Vrf(FlaskForm):
    id = StringField('ID')
    vrf_name = StringField('VRF Name', validators=[
        InputRequired(message='Ingrese VRF Name')])
    vrf_rd = StringField('VRF route distinguisher', validators=[
        InputRequired(message='Ingrese VRF RD')])
    vrf_rt = StringField('VRF route target', validators=[
        InputRequired(message='Ingrese VRF RT')])


class DeviceConfigInterfaces_vrf(FlaskForm):
    id = StringField('ID')
    interface = StringField('Interface', validators=[
                            DataRequired(message='debe ingresar una interface')])
    interface_vrf = StringField('Interface VRF', validators=[
        DataRequired(message='debe ingresar una vrf')])
    interface_ip = StringField('Interface IP', validators=[IPAddress(
        ipv4=True, ipv6=False, message='IP Destino: IP invalida')])
    interface_mascara = StringField('Interface Mascara', validators=[DataRequired(message='Si o Si debe ingresar un numero de AS'), Length(
        min=7, max=15, message='Mascara: debe tener un minimo de 5 y un maximo de 15 caracteres')])
...
class DeviceConfigInterfaces_mpls(FlaskForm):
    id = StringField('ID')
    interface = StringField('Interface', validators=[
                            DataRequired(message='debe ingresar una interface')])
    interface_ip = StringField('Interface IP', validators=[IPAddress(
...
class ebgp_vrf_form(FlaskForm):
    ...
    bgp_neighbor = StringField('BGP Neighbor', validators=[
                               InputRequired(message='Ingrese IP Neighbor')])
    bgp_vrf = StringField('BGP VRF', validators=[
        InputRequired(message='Ingrese BGP VRF')])
